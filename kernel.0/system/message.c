...
umsg32	receive(void)
{
	...
	if (active_task->prhasmsg == FALSE) {
		active_task->prstate = PR_RECV;
		yield();//resched();//PEND_SV();
	}
	msg = active_task->prmsg;		/* Retrieve message		*/
	active_task->prhasmsg = FALSE;	/* Reset message flag		*/
	restore(mask);
	return msg;
}
