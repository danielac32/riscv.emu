...
//	struct	uart_csreg *uptr;	/* Address of UART's CSRs	*/
	...
	//typtr->tyisem = sem_init(&tx,0);//semcreate(0);		/* Input semaphore	*/
	sem_init(&typtr->tyisem,0);
	typtr->tyohead = typtr->tyotail = 	/* Set up output queue	*/
		&typtr->tyobuff[0];		/*    as empty		*/
	//typtr->tyosem = sem_init(&tx,TY_OBUFLEN);//semcreate(TY_OBUFLEN);	/* Output semaphore	*/
	sem_init(&typtr->tyosem,TY_OBUFLEN);
